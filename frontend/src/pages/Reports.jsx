...
import { FileBarChart, FileSpreadsheet, Printer, FileText } from "lucide-react";
  ...
  return (
    <div>
      <PageHeader title="Reports" subtitle="Filter and export maintenance history">
        <Btn
          variant="outline"
          onClick={() => window.open(`${API}/export/maintenance.xlsx?${qs()}`, "_blank")}
        >
          <FileSpreadsheet className="h-4 w-4" /> Excel
        </Btn>
        <Btn
          variant="outline"
          onClick={() => window.open(`${API}/export/maintenance.pdf?${qs()}`, "_blank")}
        >
          <FileText className="h-4 w-4" /> PDF
        </Btn>
        <Btn variant="outline" onClick={() => window.print()} className="no-print">
      ...
      <Panel className="mb-4 p-4 no-print">
        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
          <TextInput label="Asset / SAP No." value={f.sap_no} onChange={(e) => setF({ ...f, sap_no: e.target.value })} />
          ...
          <SelectInput label="Maintenance Purpose" value={f.job_id} onChange={(e) => setF({ ...f, job_id: e.target.value })}>
            <option value="">All</option>
            {jobs.map((j) => (
              <option key={j.id} value={j.id}>
                {j.field_name || j.job_name || j.job_number}
              </option>
            ))}
          </SelectInput>
        ...
        <div className="mt-3 flex justify-end">
          <Btn onClick={run}>
            <FileBarChart className="h-4 w-4" /> Run Report
          </Btn>
        </div>
      </Panel>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-sm">
            <thead className="bg-slate-50 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-500">
              <tr>
                <th className="px-4 py-3">Maint. No.</th>
                <th className="px-4 py-3">Date</th>
                <th className="px-4 py-3">SAP</th>
                <th className="px-4 py-3">Equipment</th>
                <th className="px-4 py-3">Purpose</th>
                <th className="px-4 py-3">Failure</th>
                <th className="px-4 py-3">Lead Tech</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((m) => (
                <tr key={m.id} className="hover:bg-slate-50">
                  ...
                  <td className="max-w-[14rem] truncate px-4 py-2.5">{m.equipment_name}</td>
                  <td className="max-w-[12rem] truncate px-4 py-2.5 text-slate-600">{m.maintenance_purpose || "—"}</td>
                  <td className="max-w-[14rem] truncate px-4 py-2.5 text-slate-600">{m.failure_found || m.problem_damage}</td>
