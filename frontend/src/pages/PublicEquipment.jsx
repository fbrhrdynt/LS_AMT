...
function StatusPill({ value }) {
  ...
  const good = text.toLowerCase() === "operational";
    ...
    const load = async () => {
      ...
      try {
        const { data } = await api.get(
          `/public/equipment/${encodeURIComponent(token)}`
        );
        if (!cancelled) {
          setData(data);
          setState("ready");
      ...
      <div className="min-h-screen bg-slate-50 px-4 py-10">
        <div className="mx-auto max-w-lg rounded-xl border border-slate-200 bg-white p-8 text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            This public equipment link is invalid, expired, or has been reset.
          </p>
        </div>
      </div>
    );
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                ...
                <div className="mt-1 font-mono text-sm text-slate-500">
                  SAP {valueOrDash(equipment.sap_no)}
                </div>
              </div>

              <StatusPill value={equipment.operational_status} />
            </div>
          </div>
          ...
          <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6 lg:grid-cols-3">
            ...
            <InfoItem
              ...
              value={fmtDate(equipment.date_of_purchase)}
              mono
            />
            <InfoItem
              label="Physical Condition"
              value={equipment.physical_condition}
            />
            <div className="min-w-0">
              <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                Current Location
              ...
              <div className="mt-1 flex min-w-0 items-start gap-1.5 text-sm font-medium text-slate-800">
                ...
                <span className="break-words">
                  {valueOrDash(equipment.placement)}
          ...
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 sm:px-6">
            <div>
              ...
              <div className="mt-1 text-xs text-slate-400">
                Closed maintenance records only
              </div>
            </div>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
                        ...
                        {m.type_of_maintenance && (
                          <span>{m.type_of_maintenance}</span>
                        )}

                        {m.maintenance_category && (
                          <span>{m.maintenance_category}</span>
                        )}
                      </div>
