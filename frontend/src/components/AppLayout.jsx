...
import {
  LayoutDashboard, Wrench, Package, Building2, Briefcase, FileBarChart,
  ScrollText, Users, Search, LogOut, Menu, X, HardHat,
} from "lucide-react";
...
function GlobalSearch() {
  ...
  useEffect(() => {
    if (!q || q.length < 1) { setRes(null); return; }
  ...
  const hasResults = res && (res.equipment?.length || res.jobs?.length || res.clients?.length);
    ...
    <div className="relative w-full max-w-xl" ref={boxRef}>
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search Asset/SAP, Serial/Mfg, Equipment, Job, Client…"
          className="w-full rounded-md border border-slate-200 bg-white pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
        />
      </div>
      {open && (
        <div data-testid="global-search-results" className="absolute z-50 mt-1 w-full rounded-md border border-slate-200 bg-white shadow-lg max-h-[70vh] overflow-auto">
          {!hasResults && <div className="px-4 py-3 text-sm text-slate-500">No results</div>}
          {res?.equipment?.length > 0 && (
            <div className="py-1">
              <div className="px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Equipment</div>
              {res.equipment.map((e) => (
                <button key={e.id} data-testid={`search-eq-${e.id}`} onClick={() => go(`/equipment/${e.id}`)}
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="font-mono text-xs text-slate-500">SAP {e.sap_no} · Mfg {e.mfg_no || "—"}</div>
                  </div>
          ...
          {res?.jobs?.length > 0 && (
            <div className="py-1 border-t border-slate-100">
              <div className="px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Jobs</div>
              {res.jobs.map((j) => (
                <button key={j.id} onClick={() => go(`/jobs/${j.id}`)} className="flex w-full items-center gap-2 px-3 py-2 text-left hover:bg-slate-50">
                  <span className="font-mono text-xs text-blue-600">{j.job_number}</span>
                  <span className="text-sm text-slate-800">{j.job_name}</span>
                </button>
              ))}
            </div>
          )}
          {res?.clients?.length > 0 && (
            <div className="py-1 border-t border-slate-100">
              <div className="px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Clients</div>
              {res.clients.map((c) => (
                <button key={c.id} onClick={() => go(`/clients`)} className="flex w-full px-3 py-2 text-left text-sm text-slate-800 hover:bg-slate-50">{c.name}</button>
              ))}
            </div>
          )}
        </div>
...
export default function AppLayout({ children }) {
  ...
  const items = NAV.filter((n) => (!n.admin || isAdmin(user)) && (!n.manage || canManage(user)));
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <aside className={cn(
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div className="flex items-center gap-2.5">
            <img src="/amt-logo.webp" alt="AMT" className="h-9 w-auto object-contain" data-testid="app-logo" />
        ...
        <nav className="flex-1 space-y-0.5 overflow-y-auto p-3">
          {items.map((n) => {
            ...
            return (
              <Link key={n.key} to={n.to} data-testid={`nav-${n.key}`} onClick={() => setMobileOpen(false)}
        ...
        <div className="border-t border-slate-200 p-3">
          ...
          <button data-testid="logout-btn" onClick={logout}
            className="mt-2 flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="no-print sticky top-0 z-20 flex items-center gap-3 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur">
          <button className="lg:hidden" onClick={() => setMobileOpen(true)} data-testid="mobile-menu-btn"><Menu className="h-5 w-5" /></button>
          <GlobalSearch />
