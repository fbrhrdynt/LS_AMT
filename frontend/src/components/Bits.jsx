...
function pageExportDataset(pathname) {
  const datasets = {
    "/": "dashboard",
    "/equipment": "equipment",
...
export function PageHeader({
  ...
  const dataset =
    exportDataset === undefined
      ? pageExportDataset(location.pathname)
      : exportDataset;

  const showExport =
    !hideExport && Boolean(dataset);
...
export function EmptyState({
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center justify-center gap-2 py-12 text-slate-400",
        className
      )}
    >
...
export function Btn({
  ...
  return (
    <button
      {...props}
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-md px-3.5 py-2 text-sm font-semibold transition-colors disabled:opacity-60",
        styles[variant],
        className
      )}
    >
...
export function TextInput({
  ...
  return (
    <div className={className}>
      ...
      <input
        {...props}
        className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 text-sm outline-none transition-colors focus:ring-2 focus:ring-blue-500"
      />
    </div>
  );
}
