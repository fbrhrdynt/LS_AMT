...
async def _resolve_parts(part_lines):
    ...
    for pl in part_lines:
        ...
        resolved.append({"item_id": item["id"], "item_code": item["item_code"],
                         "item_name": item["item_name"], "type": item["type"],
                         "unit": item["unit"], "qty": pl.qty,
                         "unit_price": price, "cost": round(price * pl.qty, 2)})
    return resolved
...
async def get_maintenance(mid: str, user: dict = Depends(get_current_user)):
    ...
    m["attachments"] = await db.files.find({"maintenance_id": mid, "is_deleted": False}, {"_id": 0}).to_list(100)
    return m
...
async def create_maintenance(body: MaintenanceBody, user: dict = Depends(EDIT)):
    ...
    # auto-suggest job/client from active assignment if not provided
    ...
    doc = {
        "id": new_id(), "mnt_no": mnt_no, "equipment_id": eq["id"], "sap_no": eq["sap_no"],
        "equipment_name": eq.get("name"),
        "maintenance_date": body.maintenance_date or now_iso()[:10],
        "date_closed": None, "maintenance_category": body.maintenance_category,
        ...
        "pending_maintenance": "", "progress_update": "",
        "client_id": client_id, "client_name": client["name"] if client else None,
        "job_id": job_id, "job_number": job["job_number"] if job else None,
        "parts_consumed": parts, "total_cost": _total_cost(parts),
        "notes": body.notes, "parts_deducted": False, "attachments": [],
        "status": "Open", "source": "manual", "created_by": user["name"], "created_at": now_iso(),
        "closed_at": None,
    }
    await db.maintenance.insert_one(doc)
    await db.equipment.update_one({"id": eq["id"]}, {"$set": {"operational_status": "Under Maintenance"}})
    # record failure
    ...
    if fname:
        await db.failures.insert_one({
            ...
            "mnt_no": mnt_no, "failure_name": fname, "description": body.action_taken,
            "root_cause": body.root_cause, "occurred_date": doc["maintenance_date"],
            "created_at": now_iso()})
...
async def update_maintenance(mid: str, body: MaintenanceBody, user: dict = Depends(EDIT)):
    ...
    updates = {
        ...
        "maintenance_category": body.maintenance_category, "problem_damage": body.problem_damage,
        "failure_found": body.failure_found, "root_cause": body.root_cause,
        "action_taken": body.action_taken, "duration_days": body.duration_days,
        "lead_technician": body.lead_technician, "support_technicians": body.support_technicians,
        "checked_by": body.checked_by, "final_condition": body.final_condition, "remark": body.remark,
        "client_id": body.client_id, "client_name": client["name"] if client else None,
    ...
    if fname:
        fdoc = {"failure_name": fname, "description": body.action_taken,
                "root_cause": body.root_cause, "occurred_date": body.maintenance_date}
        if existing_f:
            await db.failures.update_one({"id": existing_f["id"]}, {"$set": fdoc})
        else:
            await db.failures.insert_one({"id": new_id(), "equipment_id": m["equipment_id"],
                                          "maintenance_id": mid, "mnt_no": m["mnt_no"],
                                          "created_at": now_iso(), **fdoc})
...
async def close_maintenance(mid: str, body: CloseBody, user: dict = Depends(EDIT)):
    ...
    for p in parts:
        ...
        if not item:
            raise HTTPException(status_code=400, detail=f"Item missing: {p['item_name']}")
        if item["stock"] < p["qty"]:
            raise HTTPException(status_code=400,
                                detail=f"Insufficient stock for {item['item_name']} (have {item['stock']}, need {p['qty']})")
    # deduct + transactions
    for p in parts:
        item = await db.inventory_items.find_one({"id": p["item_id"]})
        new_stock = item["stock"] - p["qty"]
        await db.inventory_items.update_one({"id": item["id"]}, {"$set": {"stock": new_stock, "updated_at": now_iso()}})
        await db.inventory_transactions.insert_one({
            "id": new_id(), "item_id": item["id"], "item_code": item["item_code"],
            "item_name": item["item_name"], "type": "consume", "direction": "out",
            "qty": p["qty"], "unit": item["unit"], "maintenance_id": mid, "mnt_no": m["mnt_no"],
            "equipment_id": m["equipment_id"], "balance_after": new_stock,
            "note": f"Consumed on {m['mnt_no']}", "created_by": user["name"], "created_at": now_iso()})
    updates = {"status": "Closed", "closed_at": now_iso(), "parts_deducted": True,
               "parts_consumed": parts, "total_cost": _total_cost(parts),
    ...
    await db.maintenance.update_one({"id": mid}, {"$set": updates})
    # equipment back to operational (unless other open maintenance exists)
    other_open = await db.maintenance.count_documents(
        {"equipment_id": m["equipment_id"], "status": "Open", "id": {"$ne": mid}})
    if other_open == 0:
        await db.equipment.update_one({"id": m["equipment_id"]}, {"$set": {"operational_status": "Operational"}})
    await audit_log("maintenance", mid, "maintenance.close", user,
    ...
    if m.get("parts_deducted"):
        for p in m.get("parts_consumed") or []:
            ...
            if item:
                ...
                await db.inventory_transactions.insert_one({
                    "id": new_id(), "item_id": item["id"], "item_code": item["item_code"],
                    "item_name": item["item_name"], "type": "reversal", "direction": "in",
                    "qty": p["qty"], "unit": item["unit"], "maintenance_id": mid, "mnt_no": m["mnt_no"],
                    "equipment_id": m["equipment_id"], "balance_after": new_stock,
                    "note": f"Reversal on reopen {m['mnt_no']}", "created_by": user["name"], "created_at": now_iso()})
    await db.maintenance.update_one({"id": mid}, {"$set": {"status": "Open", "parts_deducted": False, "closed_at": None, "date_closed": None}})
    await db.equipment.update_one({"id": m["equipment_id"]}, {"$set": {"operational_status": "Under Maintenance"}})
    await audit_log("maintenance", mid, "maintenance.reopen", user, f"Reopened {m['mnt_no']}")
    return await db.maintenance.find_one({"id": mid}, {"_id": 0})
        ...
        for p in m.get("parts_consumed") or []:
            ...
            if item:
                ...
                await db.inventory_transactions.insert_one({
                    ...
                    "qty": p["qty"], "unit": item["unit"], "maintenance_id": mid, "mnt_no": m["mnt_no"],
                    "equipment_id": m["equipment_id"], "balance_after": new_stock,
                    "note": f"Reversal on delete {m['mnt_no']}", "created_by": user["name"], "created_at": now_iso()})
    await db.failures.delete_many({"maintenance_id": mid})
    await db.files.update_many({"maintenance_id": mid}, {"$set": {"is_deleted": True}})
    await db.maintenance.delete_one({"id": mid})
    other_open = await db.maintenance.count_documents({"equipment_id": m["equipment_id"], "status": "Open"})
    if other_open == 0:
        await db.equipment.update_one({"id": m["equipment_id"]}, {"$set": {"operational_status": "Operational"}})
    await audit_log("maintenance", mid, "maintenance.delete", user, f"Deleted {m['mnt_no']}")
    return {"ok": True}


@router.get("/maintenance/{mid}/report.pdf")
async def maintenance_pdf(mid: str, request: Request, auth: str = Query(None), authorization: str = Header(None)):
    ...
    if not m:
        raise HTTPException(status_code=404, detail="Maintenance not found")
    eq = await db.equipment.find_one({"id": m["equipment_id"]}, {"_id": 0}) or {}
    settings = await db.settings.find_one({"_id": "app"}) or {}
    currency = settings.get("currency", "USD")
    pdf = build_maintenance_pdf(m, eq, currency)
    return Response(content=pdf, media_type="application/pdf",
                    headers={"Content-Disposition": f'inline; filename="{m["mnt_no"]}.pdf"'})


# -------- failures --------
@router.get("/failures")
async def list_failures(equipment_id: str = "", user: dict = Depends(get_current_user)):
    query = {}
    if equipment_id:
        query["equipment_id"] = equipment_id
    return await db.failures.find(query, {"_id": 0}).sort("occurred_date", -1).to_list(2000)


@router.get("/failures/recurring")
async def recurring_failures(user: dict = Depends(get_current_user)):
    pipeline = [
        {"$group": {"_id": {"$toLower": "$failure_name"},
                    "failure_name": {"$first": "$failure_name"},
                    "count": {"$sum": 1},
                    "occurrences": {"$push": {"maintenance_id": "$maintenance_id", "mnt_no": "$mnt_no",
                                              "equipment_id": "$equipment_id", "occurred_date": "$occurred_date"}}}},
        {"$sort": {"count": -1}},
    ]
    rows = await db.failures.aggregate(pipeline).to_list(2000)
    return [{"failure_name": r["failure_name"], "count": r["count"], "occurrences": r["occurrences"]}
            for r in rows if r.get("failure_name")]


# -------- files / attachments --------
@router.post("/files/upload")
async def upload_file(file: UploadFile = File(...), doc_type: str = Form("Document"),
                      equipment_id: str = Form(None), maintenance_id: str = Form(None),
    ...
    rec = {"id": new_id(), "storage_path": result["path"], "original_filename": file.filename,
           "content_type": content_type, "size": result.get("size", len(data)), "doc_type": doc_type,
           "equipment_id": equipment_id, "maintenance_id": maintenance_id, "is_deleted": False,
           "uploaded_by": user["name"], "created_at": now_iso()}
    await db.files.insert_one(rec)
    if maintenance_id:
        await db.maintenance.update_one({"id": maintenance_id}, {"$push": {"attachments": rec["id"]}})
    await audit_log("file", rec["id"], "file.upload", user, f"{file.filename} ({doc_type})")
    rec.pop("_id", None)
    return rec


@router.get("/files/{file_id}/download")
async def download_file(file_id: str, request: Request, auth: str = Query(None), authorization: str = Header(None)):
    token = await _token(request, auth, authorization)
    if not token or not await _user_from_token(token):
        raise HTTPException(status_code=401, detail="Not authenticated")
...
async def delete_file(file_id: str, user: dict = Depends(EDIT)):
    ...
    await audit_log("file", file_id, "file.delete", user, rec["original_filename"])
    return {"ok": True}
